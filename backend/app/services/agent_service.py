...
if __name__ == "__main__":
    ...
    print("\n==============================================")
    print("       PRUEBA MODO AGENTE")
    print("==============================================")

    resumen = servicio.obtener_resumen_empresa(
        empresa_id=empresa,
        pericodi=periodo
    )
    ...
    variacion = servicio.obtener_variacion_empresa(
        empresa_id=empresa,
        pericodi=periodo
    )
    ...
    desglose = servicio.obtener_desglose_variacion(
        "EMPRESA_001",
        139
    )
    ...
    impulsores = servicio.obtener_principales_impulsores(
        "EMPRESA_001",
        139
    )
    ...
    soporte_lvta = servicio.obtener_soporte_lvta(
        "EMPRESA_001",
        139
    )
    ...
    print("\n--- VALIDACIÓN TRAZABILIDAD LVTA ---")

    trazabilidad_lvta = servicio.validar_trazabilidad_lvta(
        "EMPRESA_001",
        139
    )

    print(trazabilidad_lvta)

    print("\n--- DETALLE SOPORTE LVTA JULIO ---")

    energia = servicio.energia_transferencias.copy()

    energia["vtotemtotal"] = pd.to_numeric(
        energia["vtotemtotal"],
        errors="coerce"
    )

    detalle_julio = energia[
        (energia["emprcodi"] == "EMPRESA_001")
        & (energia["pericodi"] == 138)
    ].copy()
    ...
    print("\n--- DETALLE RESULTADO LVTA JULIO ---")

    evolucion = servicio.evolucion.copy()

    evolucion["monto"] = pd.to_numeric(
        evolucion["monto"],
        errors="coerce"
    )

    detalle_lvta_julio = evolucion[
        (evolucion["empresa_deudora"] == "EMPRESA_001")
        & (evolucion["pericodi"] == 138)
        & (evolucion["proceso"] == "LVTA")
    ].copy()
    ...
    print("\n--- PERIODOS CON DATOS LSCIO ---")

    print(
        servicio.lscio_transferencias[
            [
                "emprcodi",
                "pericodi",
                "perinombre",
    ...
    print("\n--- VALIDACIÓN INTEGRIDAD LSCIO ---")

    resultado_lscio = servicio.validar_integridad_lscio(
        "EMPRESA_001",
        138
    )

    print(resultado_lscio)

    print("\n--- DETALLE LSCIO EMPRESA_001 JULIO ---")

    detalle_lscio = servicio.lscio_desglose[
        (servicio.lscio_desglose["emprcodi"] == "EMPRESA_001")
        & (servicio.lscio_desglose["pericodi"] == 138)
    ].copy()
    ...
    print("\n--- INSPECCIÓN POTENCIA EMPRESA_001 JULIO ---")

    resultado_potencia = servicio.inspeccionar_potencia(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- VALIDACIÓN TRAZABILIDAD POTENCIA EMPRESA_001 JULIO ---")

    resultado_validacion_potencia = (
        servicio.validar_trazabilidad_potencia(
            "EMPRESA_001",
            138
        )
    )
    ...
    print("\n--- DETALLE RESULTADO LVTP EMPRESA_001 JULIO ---")

    detalle_lvtp = servicio.evolucion[
        (servicio.evolucion["empresa_deudora"] == "EMPRESA_001")
        & (servicio.evolucion["pericodi"] == 138)
        & (servicio.evolucion["proceso"] == "LVTP")
    ].copy()

    detalle_lvtp["monto"] = pd.to_numeric(
        detalle_lvtp["monto"],
        errors="coerce"
    )
    ...
    print("\n--- VALIDACIÓN COMPONENTE INGRESO TARIFARIO POTENCIA ---")

    resultado_it = servicio.evolucion[
        (servicio.evolucion["empresa_deudora"] == "EMPRESA_001")
        & (servicio.evolucion["pericodi"] == 138)
        & (servicio.evolucion["proceso"] == "LVTP")
        & (
            servicio.evolucion["valorizacion"]
            == "COMPENSACIÓN A TRANSMISORAS POR INGRESO TARIFARIO"
        )
    ].copy()

    soporte_it = servicio.potencia_desglose[
        (servicio.potencia_desglose["emprcodi"] == "EMPRESA_001")
        & (servicio.potencia_desglose["pericodi"] == 138)
        & (
            servicio.potencia_desglose["valorizacion"]
            == "COMPENSACIÓN A TRANSMISORAS POR INGRESO TARIFARIO"
        )
    ].copy()

    resultado_it["monto"] = pd.to_numeric(
        resultado_it["monto"],
        errors="coerce"
    )

    soporte_it["monto"] = pd.to_numeric(
        soporte_it["monto"],
        errors="coerce"
    )

    total_resultado_it = float(
        resultado_it["monto"].sum()
    )

    total_soporte_it = float(
        soporte_it["monto"].sum()
    )
    ...
    print(
        "Soporte Potencia con signo invertido:",
        -total_soporte_it
    )

    print(
        "Diferencia:",
        total_resultado_it + total_soporte_it
    )
    ...
    print("\n--- INSPECCIÓN SST-SCT EMPRESA_001 JULIO ---")

    resultado_sstsct = servicio.inspeccionar_sstsct(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- PROCESOS EMPRESA_001 JULIO ---")

    procesos_empresa = servicio.evolucion[
        (servicio.evolucion["empresa_deudora"] == "EMPRESA_001")
        & (servicio.evolucion["pericodi"] == 138)
    ].copy()

    procesos_empresa["monto"] = pd.to_numeric(
        procesos_empresa["monto"],
        errors="coerce"
    )
    ...
    print(
        resumen_procesos.to_string(index=False)
    )

    print("\n==============================================")
    print("       CAUSAS DE VARIACIÓN LSCIO")
    print("==============================================")

    resultado = servicio.obtener_causas_variacion_lscio(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- CAUSAS VARIACIÓN LVTA ---")

    resultado = servicio.obtener_causas_variacion_lvta(
        "EMPRESA_001",
        139
    )
    ...
    print("\n--- VALIDACIÓN RECONSTRUCCIÓN LSCIO ---")

    resultado = servicio.validar_reconstruccion_lscio(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- CAUSAS POR CONCEPTO LSCIO ---")

    resultado = servicio.obtener_causas_concepto_lscio(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- VALIDACIÓN RECONSTRUCCIÓN CONCEPTOS LSCIO ---")

    resultado = servicio.validar_reconstruccion_conceptos_lscio(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- EVIDENCIA CONCEPTO LSCIO ---")

    resultado = servicio.obtener_evidencia_concepto_lscio(
        "EMPRESA_001",
        138,
        "Inflexibilidad Operativa",
        "Pago - CCV Por Seguridad"
    )

    print(resultado)

    print("\n--- EXPLICACIÓN COMPLETA LSCIO ---")

    resultado = servicio.obtener_explicacion_lscio(
        "EMPRESA_001",
        138
    )
    ...
    print("\n--- COMPARACIÓN DE SIGNOS LSCIO ---")

    empresa = "EMPRESA_001"
    pericodi = 138

    resultado_resumen = servicio.obtener_resumen_empresa(
        empresa,
        pericodi
    )

    resultado_lscio = servicio.obtener_causas_variacion_lscio(
        empresa,
        pericodi
    )

    print("\nRESUMEN EMPRESA:")
    print(resultado_resumen["procesos"])

    print("\nCAUSAS LSCIO:")
    print(resultado_lscio["resumen"])

    print("\nLSCIO TRANSFERENCIAS:")
    df_lscio = servicio.lscio_transferencias

    fila_transferencia = df_lscio[
        (df_lscio["emprcodi"] == empresa)
        & (df_lscio["pericodi"] == pericodi)
    ]

    for _, fila in fila_transferencia.iterrows():
        print("Empresa:", fila["emprcodi"])
        print("Periodo:", fila["pericodi"])
        print("Monto total:", fila["monto_total"])
        print("Periodo nombre:", fila["perinombre"])
        print("Preliminar:", fila["periodo_preliminar"])

    print("\n--- VALIDACIÓN DE SIGNOS LSCIO: JUNIO VS JULIO ---")

    empresa = "EMPRESA_001"

    for pericodi in [137, 138]:
        ...
        fila_transferencia = servicio.lscio_transferencias[
            (servicio.lscio_transferencias["emprcodi"] == empresa)
            & (servicio.lscio_transferencias["pericodi"] == pericodi)
        ]

        print(
            "Resultado liquidación LSCIO:",
            [
                x["monto"]
                for x in resultado_resumen["procesos"]
                if x["proceso"] == "LSCIO"
            ]
        )
    ...
    print("\n--- EXPLICACIÓN CONSOLIDADA DE EMPRESA ---")

    empresa = "EMPRESA_001"
    pericodi = 138

    resultado = servicio.obtener_explicacion_empresa(
        empresa,
        pericodi
    )
    ...
    print("\nPERIODO:")
    print(resultado["periodo"])

    print("\nRESULTADO:")
    print(resultado["resultado"])

    print("\nVARIACIÓN:")
    print(resultado["variacion"])

    print("\nIMPULSORES:")
    print(resultado["impulsores"])

    print("\n--- EXPLICACIÓN COMPLETA DE EMPRESA ---")
    ...
    print("\n===== LVTA =====")
    print(resultado["explicaciones"]["LVTA"])

    print("\n===== LSCIO =====")
    print(resultado["explicaciones"]["LSCIO"])

    print("\n===== LVTP =====")
    print(resultado["explicaciones"]["LVTP"])

    print("\n===== SST-SCT =====")
    print(resultado["explicaciones"]["SST-SCT"])

    print("\n===== TRAZABILIDAD LVTP =====")

    resultado = servicio.validar_trazabilidad_lvtp(
        "EMPRESA_001",
        138
    )
    ...
    print("\n===== TRAZABILIDAD EMPRESA =====")

    resultado = servicio.obtener_trazabilidad_empresa(
        "EMPRESA_001",
        138
    )
